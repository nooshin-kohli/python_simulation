...
def Plot_base_coordinate(cr,q,qdot):
    ...
    for i in range(len(cr.t)):
        ...
        xx, xxdot = cr.CalcBodyToBase(cr.model.GetBodyId('trunk'),np.zeros(3), True, index = i,q=q,qdot=qdot)
...
# q = np.zeros((1,3)) # joint position
# q[-1,2] = -0.4
# # q = [0,0,0,0]
# # qdot = [0,0,0,0]
# # q[3]=0.3
# qdot = np.zeros((1,3)) #joint velocity
# u = np.zeros(3) # control inputs
# # u[0] = 10000
